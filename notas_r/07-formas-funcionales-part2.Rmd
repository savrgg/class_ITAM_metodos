...
```
```{r}
sum(datos_est$horas_estudiadas_est*datos_est$calificacion_est)
mean(datos_est$horas_estudiadas_est)
mean(datos_est$calificacion_est)
var(datos_est$horas_estudiadas_est)
...
```{r}
datos_log <- datos %>% 
  mutate(horas_estudiadas_log = log(horas_estudiadas),
         calificacion_log = log(calificacion))

datos_log

model_est <- 
  linear_reg() %>% 
  fit(calificacion_log ~ horas_estudiadas_log, data= datos_log)
```


# con logaritmo una y otra
```{r}
datos_log1 <- datos %>% 
  mutate(horas_estudiadas_log = log(horas_estudiadas))
...
model_est <- 
  linear_reg() %>% 
  fit(calificacion ~ horas_estudiadas_log, data= datos_log1)
```

```{r}
datos_log2 <- datos %>% 
  mutate(calificacion_log = log(calificacion))

datos_log2

model_est <- 
  linear_reg() %>% 
  fit(calificacion_log ~ horas_estudiadas, data= datos_log2)
```
